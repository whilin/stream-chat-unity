...
namespace StreamChat.SampleProject {
    ...
    public class ChatState : IChatState {
        ...
        public Task<ChannelState> CreateNewChannelAsync (string channelName) => Client.ChannelApi.GetOrCreateChannelAsync (channelType: "messaging", channelId : Guid.NewGuid ().ToString (),
        ...
        public async Task UpdateChannelsAsync () {
            var request = new QueryChannelsRequest {
                ...
                {
                "members",
                new Dictionary<string, object> { { "$in", new string[] { Client.UserId } }
                }
                }
                },
            ...
            try {
                ...
                /*
                List<ChannelState> newList = new List<ChannelState>();
                //Jongok 
                queryChannelsResponse.Channels.ForEach( channel => {
                    var prev = _channels.Find(q => q.Channel.Id == channel.Channel.Id);
                    if(prev !=null)
                        newList.Add(prev);
                    else
                        newList.Add(channel);
...
*/
        ...
        private async void OnClientConnected () {
            UnityEngine.Debug.Log ($"[Verbose] ChatState OnClientConnected");
            ...
            if (previous != null) {
                var channel = GetChannel (previous.Channel.Id);
                if (channel != null) {
                    UnityEngine.Debug.Log ($"[Verbose] ChatState OnClientConnected previous resetting");
                    ActiveChannel = channel;
                }
            }

            if (ActiveChannel == null && _channels.Count > 0) {
                UnityEngine.Debug.Log ($"[Verbose] ChatState OnClientConnected first channel selected");
        ...
        private void OnMessageReceived (EventMessageNew messageNewEvent) {
            UnityEngine.Debug.Log ($"[Verbose] ChatState OnMessageReceived {messageNewEvent.ChannelId}  msg:{messageNewEvent.Message.Text}");
